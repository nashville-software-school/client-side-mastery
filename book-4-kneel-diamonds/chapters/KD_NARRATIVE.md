...
```js
/*
    Use array deconstruction to get each object from the
    inventory arrays assigned to a variable.
*/
const [classic, modern, vintage] = ringStyles
const [silver, gold14, gold24, platinum, palladium] = metals
const [xsmall, small, medium, large, xlarge] = diamonds

/*
    Invoke the function and give it the information it
    needs to create a new object. Assign the object that
    is returned as the value of the `newRingOrder` variable.
*/
const newRingOrder = createCustomRing(modern, medium, gold24)
